...
Copied forward (paths only) from
`tasks/t0015_v11_duration_blowup_forensics/code/audio_quality_check.py` for t0016 v3 recipe
recovery (Milestone 3, REQ-3/REQ-11) -- gate logic is unmodified, per this module's own documented
backward-compatibility contract (do not fold `duration_sanity_pass`/`longest_nonsilent_run_s` into
`is_likely_noise`). `demo()` below is repointed at t0016-local fixtures instead of t0015's
v10/v11-specific ones.

t0015's own history note, preserved verbatim: copied forward (paths only) from
...
def demo() -> None:
    """Runnable self-check (one assert-based check for this module's branch logic, not a full test
    suite) -- asserts the heuristic classifies a known-good ElevenLabs David reference clip as not
    noise. t0016 has no local known-noise fixture of its own (unlike t0015's v10/v11 clips), so this
    demo only exercises the known-good side; the real gate scoring for this task is the per-epoch
    and shipped-bundle tables in `results/v3_checkpoint_forensics.md` (Milestone 3, REQ-3/REQ-11).
    """
    from tasks.t0016_v3_recipe_recovery.code.paths import ELEVENLABS_DAVID_DIR

    candidates = sorted(ELEVENLABS_DAVID_DIR.glob("*.wav")) if ELEVENLABS_DAVID_DIR.exists() else []
    if len(candidates) == 0:
        print("demo() requires t0008's data/11labs_david/ -- run `dvc pull` in this task first.")
        return

    control_wav = candidates[0]
    control_result = check_audio_quality(control_wav)
    print(f"control ({control_wav.name}): {control_result}")

    assert not control_result.is_likely_noise, (
        f"known-good ElevenLabs reference clip misclassified as noise: {control_result}"
    )
    print("demo() OK: heuristic correctly classifies the known-good ElevenLabs reference clip.")
