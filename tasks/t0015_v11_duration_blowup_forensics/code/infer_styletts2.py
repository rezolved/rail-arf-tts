"""Instrumented StyleTTS2-native inference harness, copied forward for t0015 Milestone B
(REQ-1's mandatory `pred_dur`/frame-count instrumentation) and Milestone C (REQ-5/REQ-6/REQ-7's
inference-parameter sweep).

Adapted from `tasks/t0014_v11_decoder_fix_retrain/code/infer_styletts2.py` (paths only, no logic
changes there), itself adapted from
`tasks/t0013_v10_synthesis_quality_forensics/code/infer_styletts2.py`, itself adapted from
`code/kikiri-tts/StyleTTS2/Demo/Inference_LibriTTS.ipynb` (cells 2-16). Unlike
`kokoro.KModel`/`KPipeline` (confirmed broken for a `hifigan`-decoder checkpoint), this goes through
StyleTTS2's own native `models.py`, which dispatches `Decoder` construction on
`model_params.decoder.type` (`istftnet` or `hifigan`, `models.py:build_model`) -- the correct path
for the v11 checkpoint.

`synthesize()` is instrumented (plan.md Milestone B step 4, REQ-1) to capture and return the raw
`pred_dur` per-token values, the resulting frame count (`pred_dur.sum()`), and the input token
count directly -- the duration blowup investigated by this task must be measured from these
numbers, not inferred from output audio length alone. `alpha`/`beta`/`embedding_scale` are also
now exposed as CLI flags (plan.md Milestone C step 8, REQ-5) alongside the pre-existing
`--diffusion-steps`, so the parameter sweep can drive `synthesize()` either via CLI or by importing
the function directly from a driver script.
...
# This script is copied verbatim onto the VM (tasks/t0014.../code/, under the persist-backed
# kokoro-finetune checkout -- no full git repo present there), so it deliberately does NOT import
# t0014's own code/paths.py cross-task-style constants for STYLETTS2_DIR -- it takes
# --styletts2-dir as a CLI arg instead (default: this file's own StyleTTS2 checkout location when
# run locally, i.e. two levels up == kokoro-finetune root on the VM).
_DEFAULT_STYLETTS2_DIR = Path(__file__).resolve().parents[3]
...
class SynthesisResult:
    """Result of one `synthesize()` call, instrumented per plan.md Milestone B step 4 (REQ-1).

    `pred_dur`/`pred_dur_sum`/`input_token_count` are logged directly from the forward pass -- not
    inferred from `wav`'s length -- so callers can distinguish a predictor-calibration failure
    (anomalous `pred_dur` values) from a plumbing bug (a normal `pred_dur` distribution feeding an
    anomalous `pred_aln_trg` frame count) or a phonemizer over-segmentation issue (anomalous
    `input_token_count`).
    """
...
def synthesize(
    ...
    *,
    ...
    """Returns a `SynthesisResult`. Adapted from notebook cell 16."""
    import phonemizer
    ...
    with torch.no_grad():
        ...
        # Instrumentation (plan.md Milestone B step 4, REQ-1): capture pred_dur's raw per-token
        # values and the resulting frame count directly from the forward pass, before they are
        # consumed by pred_aln_trg construction below -- this is the ground truth for localizing
        # the duration blowup, not something to be inferred from wav length afterward.
        pred_dur_list: list[float] = pred_dur.detach().cpu().tolist()
        if isinstance(pred_dur_list, float):  # a single-token input squeezes to a 0-d tensor
            pred_dur_list = [pred_dur_list]
        pred_dur_sum: int = int(pred_dur.sum().item())
        input_token_count: int = int(input_lengths.item())
...
def main() -> None:
    ...
    parser.add_argument("--styletts2-dir", type=Path, default=_DEFAULT_STYLETTS2_DIR)
    ...
    styletts2_dir = args.styletts2_dir.resolve()
    ...
    sf.write(str(output_wav), result.wav, SAMPLE_RATE)
    duration_s = len(result.wav) / SAMPLE_RATE
    rtf = result.wall_time_seconds / duration_s if duration_s > 0 else None
    print(
        f"Wrote {output_wav} ({duration_s:.2f}s audio, synth wall time "
        f"{result.wall_time_seconds:.2f}s, rtf={rtf}, pred_dur_sum={result.pred_dur_sum}, "
