...
def load_checkpoint(model, optimizer, path, load_only_params=True, ignore_modules=[]):
    """DP-aware loader that shadows models.load_checkpoint and fails loudly on key mismatch.

    Upstream loads with strict=False, so a checkpoint saved from a DataParallel-wrapped model
    (every key prefixed "module.") loads ZERO parameters while still printing "<key> loaded" —
    silently training Stage 2 from scratch instead of from Stage 1.
    """
...
def main(config_path):
    ...
    # ponytail: baseline voicepack extraction skipped (rglob scans .venv, hangs for hours)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(pbar):
            ...
            if (i + 1) % log_interval == 0:
                pbar.set_postfix(loss=f"{running_loss/log_interval:.4f}", dur=f"{loss_dur:.3f}", f0=f"{loss_F0_rec:.3f}")
                logger.info(
        ...
        _ = [model[key].eval() for key in model]
        # ponytail: per-epoch voicepack extraction skipped (rglob scans .venv, hangs)
        _ = [model[key].train() for key in model]
        # ─────────────────────────────────────────────────────────────────────
