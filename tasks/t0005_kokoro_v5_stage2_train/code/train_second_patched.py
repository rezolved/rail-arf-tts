...
torch.autograd.set_detect_anomaly(False)  # was True — aborted backward() before our
# gradient-clipping/finite-check code could ever run, on the very first untrained-discriminator
# backward pass at joint_epoch activation. Disabled so backward() completes and the finite-check
# below can skip a bad step instead of the whole process crashing.
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if start_ds:
                ...
                if _grads_finite(model.msd, model.mpd):
                    ...
                    print(f"[skip] non-finite grad in msd/mpd at epoch {epoch} step {i} — skipping this discriminator update")
                    optimizer.zero_grad()
            else:
                d_loss = 0
            ...
            if loss_mel is not None and torch.isnan(loss_mel).any():
                print(f"[skip] NaN in loss_mel at epoch {epoch} step {i} — skipping this batch entirely")
                optimizer.zero_grad()
                ...
                if _CONSECUTIVE_SKIPS["n"] >= _MAX_CONSECUTIVE_SKIPS:
                    print(f"[ABORT] {_CONSECUTIVE_SKIPS['n']} consecutive skipped batches — "
                          f"the model is not training any more, stopping instead of spinning.", flush=True)
                    import sys
                    sys.exit(2)
                continue
            if start_ds:
            ...
            gen_modules = [model.bert_encoder, model.bert, model.predictor,
                           model.predictor_encoder, model.diffusion, model.style_encoder,
                           model.decoder]
            if not _grads_finite(*gen_modules):
                print(f"[skip] non-finite grad in generator group at epoch {epoch} step {i} — skipping this generator update")
                optimizer.zero_grad()
            else:
                if train_LM:
