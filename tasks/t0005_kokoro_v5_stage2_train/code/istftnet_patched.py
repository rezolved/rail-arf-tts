...
class Generator(torch.nn.Module):
    ...
    def forward(self, x, s, f0):
        ...
        x = F.leaky_relu(x)
        x = self.conv_post(x)
        spec = torch.exp(torch.clamp(x[:, : self.post_n_fft // 2 + 1, :], max=15.0))  # clamp: unbounded exp() here blows the decoder output to ~1e22 on the first GAN-gradient step at joint_epoch, overflowing fp32 in the mel power spectrogram (see t0005 logs). e^15 is far above any valid magnitude, so this never binds in normal operation.
