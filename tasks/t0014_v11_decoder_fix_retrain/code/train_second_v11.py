"""Kokoro Stage 2 v11 training script with safeguards (REQ-3, REQ-6, REQ-7, REQ-8).

Copied forward unchanged from tasks/t0010_stage2_safeguarded_training/code/train_second_v10.py
(t0010's completed task folder is never modified -- corrections convention). Added safeguards:
...
v11 decoder-init fix (REQ-1, plan.md Milestone A step 2): `load_checkpoint()`'s `ignore_modules`
list below is INTENTIONALLY LEFT UNCHANGED from v10 (`["predictor_encoder", "msd", "mpd", "wd",
"diffusion"]`, still omitting "decoder"). t0013's forensics
(tasks/t0013_v10_synthesis_quality_forensics/results/v10_diagnosis.md) proved v10's bug was never
the ignore list itself -- it was that `config_david_v10.yml`'s `first_stage_path` pointed at
`first_stage_v3.pth`, an ISTFTNet-shaped checkpoint, which silently partial-shape-matched into this
config's HiFi-GAN-shaped `decoder` block (early architecture-agnostic layers only), leaving the
real vocoder layers (`ups`, `resblocks`, `alphas`, `conv_post`, `noise_convs`) at random init --
worse than true random init per t0013's falsification probe. The actual fix lives entirely in
`config_david_v11.yml`'s `first_stage_path`, which now points at `yl4579/StyleTTS2-LibriTTS`'s
`epochs_2nd_00020.pth` -- a checkpoint whose `decoder` is genuinely HiFi-GAN-shaped (confirmed by
this task's own tensor-level pre-flight check, `results/checkpoint_forensics_v11.md`, BEFORE any
GPU spend). Do NOT "fix" this list by adding "decoder" to it -- that would silently reintroduce
random-decoder-init training (this plan's rejected Approach 2 fallback), not fix anything.

Usage (same CLI as the original):
    python train_second_v11.py \
...
def _rename_legacy_parametrization_keys(sd: dict) -> dict:
    ...
    v11 addition (found live, during this task's own Milestone B launch attempt -- NOT anticipated
    by plan.md's pre-flight tensor check, which only verified decoder *architecture* classification,
    not key-name-level load compatibility against `build_model()`'s actual state dict). Without this
    rename, `yl4579/StyleTTS2-LibriTTS`'s `epochs_2nd_00020.pth` -- saved under the classic
    (pre-parametrize-API) normalization wrappers -- only shape/name-matched 438/678 decoder params,
    90/122 predictor params, and just 16/67 style_encoder params on the first launch attempt: a
    second silent partial-load, this time from a naming-convention drift rather than an architecture
    mismatch. `tasks/t0013_v10_synthesis_quality_forensics/code/infer_styletts2.py` already
    discovered and fixed this exact issue for its own (inference-only) loader
    (`_rename_legacy_parametrization_keys`, same logic copied verbatim here) when validating
    this same checkpoint as a "known-good control" -- this task's `load_checkpoint()` (inherited
    unchanged from v10) never had it, because v10 never got far enough to notice (its own bug --
    an architecturally mismatched checkpoint -- masked this one entirely). Confirmed empirically
    (`_rename_legacy_parametrization_keys` docstring in `infer_styletts2.py`) both renames are
    pure (shape-preserving, no numerical transform needed). This project's own DataParallel-
    trained checkpoints (any v11 self-resume) already use the new naming, so this rename is a
    no-op for them.
    """
...
def load_checkpoint(model, optimizer, path, load_only_params=True, ignore_modules=None):
    """DP-aware loader: strips 'module.' prefix, renames legacy parametrization keys, raises on
    0-param match.
    ...
    for key in model:
        ...
        ckpt_sd = _rename_legacy_parametrization_keys(ckpt_sd)
    ...
    if not load_pretrained:
        if config.get("first_stage_path", "") != "":
            ...
            model, _, start_epoch, iters = load_checkpoint(
                ...
                # v11: unchanged from v10 -- "decoder" belongs OFF this list. The v10 bug was a
                # mismatched first_stage_path checkpoint, not this list; see module docstring.
                ignore_modules=[
                    "predictor_encoder",
                    "msd",
                    "mpd",
