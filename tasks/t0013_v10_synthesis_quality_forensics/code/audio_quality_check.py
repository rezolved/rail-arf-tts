...
Milestone F deliverable (REQ-12). This is the objective, automatable substitute for "I listened and
it sounded like X" used throughout this task's `control_test.md` / `v10_diagnosis.md`: a training
task should run this on a handful of smoke-test synthesis clips BEFORE claiming `completed`, closing
the gap that let `kokoro-v10-best` ship with nobody having listened to its audio (see
`task_description.md` Motivation).
...
def demo() -> None:
    """Runnable self-check (ponytail: one assert-based check for this module's branch logic, not a
    full test suite) -- asserts the heuristic classifies a known-noise v10 clip and a known-good
    control clip correctly, using this task's own audio deliverables.
    """
    from tasks.t0013_v10_synthesis_quality_forensics.code.paths import RESULTS_AUDIO_DIR

    control_wav = RESULTS_AUDIO_DIR / "control_epochs_2nd_00020.wav"
    v10_wav = RESULTS_AUDIO_DIR / "v10_epoch16_primary.wav"

    if not control_wav.exists() or not v10_wav.exists():
        print("demo() requires results/audio_samples/ to be populated -- run Milestones D/E first.")
        return
